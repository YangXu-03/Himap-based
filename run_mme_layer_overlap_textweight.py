...
def main():
    ...
    for category, sample in tqdm(samples.items(), desc="Processing categories"):
        ...
        for layer_idx, layer_attn in enumerate(attentions):
            ...
            # 1. Calculate Text Token Importance
            text_start = image_end
            if text_start >= seq_len:
                # Fallback if no text tokens (e.g. empty output/question?)
                image_attn_scores = torch.zeros(image_token_len, device=layer_attn.device)
            else:
                # Text-to-Text Attention: (batch, heads, text_len, text_len)
                # Slicing from text_start to end for both Query and Key
                text_to_text = layer_attn[:, :, text_start:, text_start:]
                
                # Compute Text Importance: Sum aggregation over heads and queries (dim=2)
                # This calculates how much attention each text token RECIEVES (Key Importance / In-degree)
                # Result shape: (batch, text_len)
                text_importance_raw = text_to_text.sum(dim=1).sum(dim=2)
                
                # Normalize importance to sum to 1 per batch
                text_importance = text_importance_raw / (text_importance_raw.sum(dim=-1, keepdim=True) + 1e-9)
                
                # 2. Calculate Weighted Text-to-Image Attention
                # Text-to-Image Attention: (batch, heads, text_len, image_len)
                # Query: Text tokens, Key: Image tokens
                image_start = sys_length
                text_to_image = layer_attn[:, :, text_start:, image_start:image_end]
                
                # Sum over heads -> (batch, text_len, image_len)
                text_to_image_sum = text_to_image.sum(dim=1)
                
                # Apply weights
                # text_importance: (batch, text_len) -> (batch, text_len, 1) for broadcasting
                # We weight the contribution of each text token (as a query) by its importance
                weighted_text_to_image = text_to_image_sum * text_importance.unsqueeze(-1)
                
                # Sum over text tokens to get final score for each image token
                # Result shape: (batch, image_len) -> [0] for first item
                image_attn_scores = weighted_text_to_image.sum(dim=1)[0]
