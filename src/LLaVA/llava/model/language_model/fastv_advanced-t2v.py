...
class FastvAdvanced_LlamaModel(LlamaModel):
    ...
    def _select_tokens_max_head(self, attention_weights, sys_length, image_token_length, attention_rank):
        ...
        Uses user query tokens (after image tokens) to compute head importance.
        
        Args:
            attention_weights: [batch, num_heads, seq_len, seq_len]
            sys_length: Starting position of image tokens (length of system prompt)
        ...
        # User query tokens start after [system_prompt + image_tokens]
        user_query_start = sys_length + image_token_length
        
        # Get attention from user query tokens to image tokens: [batch, num_heads, user_query_len, image_token_length]
        text_to_image_attention = attention_weights[:, :, user_query_start:, sys_length:sys_length+image_token_length]
        
        # Sum attention across user query tokens and image tokens for each head: [batch, num_heads]
        head_importance = text_to_image_attention.sum(dim=(2, 3))
        
        # Find the head with maximum attention to vision tokens
        max_head_idx = head_importance.argmax(dim=1)  # [batch]
        
        # Get attention from the max head (average across all user query tokens): [batch, image_token_length]
        max_head_text_to_image = torch.stack([
            text_to_image_attention[b, max_head_idx[b], :, :] for b in range(batch_size)
        ])  # [batch, user_query_len, image_token_length]
        
        # Average across user query tokens: [batch, image_token_length]
        max_head_attention = max_head_text_to_image.mean(dim=1)
    ...
    def _select_tokens_avg_all_heads(self, attention_weights, sys_length, image_token_length, attention_rank):
        """
        Strategy 2: Select tokens based on average attention across all heads and all user query tokens
        ...
        # Average across all heads and all user query tokens: [batch, image_token_length]
        avg_image_attention = text_to_image_attention.mean(dim=(1, 2))
        
        # Select top-k tokens
        if attention_rank > 0:
            top_indices = avg_image_attention[0].topk(attention_rank).indices
        else:
            top_indices = torch.tensor([], dtype=torch.long, device=attention_weights.device)
    ...
    def _select_tokens_weighted_combination(self, attention_weights, sys_length, image_token_length, attention_rank):
        """
        Strategy 3: Weighted combination of max_head and avg_all_heads methods
        score = max_head_attention * alpha + avg_all_heads_attention * (1 - alpha)
        ...
        # Get attention from user query tokens to image tokens
        text_to_image_attention = attention_weights[:, :, user_query_start:, sys_length:sys_length+image_token_length]
        
        # Compute max_head attention: [batch, image_token_length]
        # Sum attention across user query tokens and image tokens for each head
        head_importance = text_to_image_attention.sum(dim=(2, 3))  # [batch, num_heads]
        max_head_idx = head_importance.argmax(dim=1)  # [batch]
        
        # Get attention from max head and average across user query tokens
        max_head_text_to_image = torch.stack([
            text_to_image_attention[b, max_head_idx[b], :, :] for b in range(batch_size)
        ])  # [batch, user_query_len, image_token_length]
        max_head_attention = max_head_text_to_image.mean(dim=1)  # [batch, image_token_length]
        
        # Compute avg_all_heads attention: [batch, image_token_length]
        avg_all_heads_attention = text_to_image_attention.mean(dim=(1, 2))
        
        # Weighted combination
        alpha = self.weighted_alpha
        combined_attention = alpha * max_head_attention + (1 - alpha) * avg_all_heads_attention
    ...
    def _select_tokens_text_weighted(self, attention_weights, sys_length, image_token_length, attention_rank):
        ...
        Use user query token self-attention to compute token importance, then weight the
        query-to-vision attention by token importance before selecting tokens.
        ...
        # Extract user query self-attention (attention between user query tokens)
        text_start = user_query_start
        text_end = seq_len
        
        # Get user query self-attention: [batch, num_heads, num_query_tokens, num_query_tokens]
        text_to_text_attention = attention_weights[:, :, text_start:text_end, text_start:text_end]
        
        # Compute user query token importance: sum of attention each token receives from all other tokens
        # Sum over the source dimension (dim=2) and average across all heads
        # [batch, num_query_tokens]
        text_importance = text_to_text_attention.sum(dim=2).mean(dim=1)
        
        # Normalize text importance (softmax to get importance distribution)
        text_importance = torch.softmax(text_importance, dim=-1)  # [batch, num_query_tokens]
        
        # Now compute user query to vision attention for each head
        # Get attention from user query tokens to vision tokens: [batch, num_heads, num_query_tokens, image_token_length]
        text_to_vision_attention = attention_weights[:, :, user_query_start:, sys_length:sys_length+image_token_length]
        
        # Weight by user query token importance: [batch, num_heads, num_query_tokens, image_token_length]
        # Expand text_importance: [batch, 1, num_query_tokens, 1]
        text_importance_expanded = text_importance.unsqueeze(1).unsqueeze(-1)
        
        # Weighted attention: [batch, num_heads, num_query_tokens, image_token_length]
        weighted_text_to_vision = text_to_vision_attention * text_importance_expanded
        
        # Sum over user query tokens: [batch, num_heads, image_token_length]
    ...
    def _select_tokens_text_weighted_max_head(self, attention_weights, sys_length, image_token_length, attention_rank):
        ...
        Use user query token self-attention to compute token importance, weight the
        query-to-vision attention for each head, find the head with maximum weighted
        ...
        # Weight by user query token importance for each head
        # Expand text_importance: [batch, 1, num_query_tokens, 1]
        text_importance_expanded = text_importance.unsqueeze(1).unsqueeze(-1)
        ...
        # Sum over user query tokens to get per-head weighted vision attention: [batch, num_heads, image_token_length]
        ...
        max_head_idx = head_total_weighted_attention.argmax(dim=1)  # [batch]
    ...
    def _generate_attention_mask_with_selection(
        ...
        # DEBUG: Print strategy on first selection (only once per forward pass)
        if not hasattr(self, '_debug_printed'):
            import sys
            print(f"[DEBUG] Token selection method: {self.token_selection_method}", file=sys.stderr)
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                # Note: Don't redefine variables here, use the ones defined at the beginning of the loop
