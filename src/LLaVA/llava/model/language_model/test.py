...
class Fastv_LlamaModel(LlamaModel):

    def __init__(self, config: LlamaConfig):
        super().__init__(config)

        self.config = config
        # fastv hyperparameter
        ...
        # 保存最近一次生成的 fastv mask / kept indices（供外部读取）
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if USE_FAST_V:
                    if idx < AGG_LAYER:
                        ...
                        if idx != 0:
                            # fetch attention output (may be Tensor or tuple/list)
                            ...
                            # If we didn't get an attention Tensor, fall back to no pruning
                            if last_layer_attention is None:
                                ...
                                # Handle edge case: ATTENTION_RANK = 0 means complete removal
                                if ATTENTION_RANK > 0:
                                ...
                                # 保存原始 bool mask 与 kept indices（相对于整个序列的全局索引）
                                try:
                                    # 保存到 cpu 以便外部读取
