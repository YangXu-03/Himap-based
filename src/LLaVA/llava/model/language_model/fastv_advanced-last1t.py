...
class FastvAdvanced_LlamaModel(LlamaModel):
    ...
    def _select_tokens_max_head(self, attention_weights, sys_length, image_token_length, attention_rank):
        ...
        """
        # Get attention from last prompt token to all positions: [batch, num_heads, seq_len]
        seq_len = attention_weights.shape[2]
        _, _, prompt_last_index = self._get_prompt_span(seq_len, sys_length, image_token_length)
        last_token_attention = attention_weights[:, :, prompt_last_index, :]
    ...
    def _select_tokens_avg_all_heads(self, attention_weights, sys_length, image_token_length, attention_rank):
        ...
        # Get attention from last prompt token to image tokens: [batch, image_token_length]
        seq_len = attention_weights.shape[2]
        _, _, prompt_last_index = self._get_prompt_span(seq_len, sys_length, image_token_length)
        last_token_image_attention = avg_attention[0, prompt_last_index, sys_length:sys_length+image_token_length]
    ...
    def _select_tokens_text_weighted(self, attention_weights, sys_length, image_token_length, attention_rank):
        ...
        # Get attention from last prompt token: [batch, num_heads, seq_len]
        prompt_start, prompt_end, prompt_last_index = self._get_prompt_span(
            seq_len, sys_length, image_token_length
        )
        last_token_attention = attention_weights[:, :, prompt_last_index, :]
        
        # Extract text-to-text attention (from last token to text tokens)
        # Text tokens are in the prompt span (after system + vision)
        text_start = prompt_start
        text_end = prompt_end
        
        # Get text-to-text attention: [batch, num_heads, num_text_tokens]
        text_attention = last_token_attention[:, :, text_start:text_end]
        
        # Average across all heads to get text importance: [batch, num_text_tokens]
        text_importance = text_attention.mean(dim=1)  # [batch, num_text_tokens]
        
        # Normalize text importance (softmax to get importance distribution)
        text_importance = torch.softmax(text_importance, dim=-1)  # [batch, num_text_tokens]
        ...
        text_to_vision_attention = attention_weights[:, :, text_start:text_end, sys_length:sys_length+image_token_length]
    ...
    def _select_tokens_text_weighted(self, attention_weights, sys_length, image_token_length, attention_rank):
        ...
        # Average across all heads to get text importance: [batch, num_text_tokens]
        text_importance = text_attention.mean(dim=1)
