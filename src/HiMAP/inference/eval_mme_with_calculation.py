...
def export_for_calculation(results, output_dir):
    os.makedirs(output_dir, exist_ok=True)

    task_to_items = defaultdict(list)
    for i, item in enumerate(results):
        pair_key = clean_text_field(item.get("question_id", ""))
        if not pair_key:
            pair_key = clean_text_field(item["image_file"])
        task_to_items[item["category"]].append((
            pair_key,
            clean_text_field(item["image_file"]),
            clean_text_field(item.get("question", "")),
            clean_text_field(item.get("gt", "")).lower(),
            clean_text_field(item.get("pred", "")).lower(),
            i,
        ))

    expected_tasks = [task for tasks in MME_TASKS.values() for task in tasks]
    for task in expected_tasks:
        if task not in task_to_items or len(task_to_items[task]) == 0:
            raise ValueError(f"Task '{task}' has no predictions, cannot run calculation.py")

        by_pair = defaultdict(list)
        for one in task_to_items[task]:
            by_pair[one[0]].append(one)

        output_lines = []
        for pair_key in sorted(by_pair.keys()):
            samples = sorted(by_pair[pair_key], key=lambda x: x[5])
            if len(samples) != 2:
                raise ValueError(
                    f"Task '{task}' pair '{pair_key}' has {len(samples)} questions, "
                    "but calculation.py expects exactly 2 questions per pair."
                )
            for sample in samples:
                _, image, question, gt_ans, pred_ans, _ = sample
                output_lines.append(f"{image}\t{question}\t{gt_ans}\t{pred_ans}\n")

        task_file = os.path.join(output_dir, f"{task}.txt")
        with open(task_file, "w", encoding="utf-8") as f:
            f.writelines(output_lines)


def run_calculation(calculation_script, results_dir):
    cmd = [sys.executable, calculation_script, "--results_dir", results_dir]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--jsd-entropy-n-base-192', type=float, required=False, help='phase-1 base keep count when target tokens = 192')
    parser.add_argument('--jsd-entropy-n-base-128', type=float, required=False, help='phase-1 base keep count when target tokens = 128')
    parser.add_argument('--jsd-entropy-n-base-64', type=float, required=False, help='phase-1 base keep count when target tokens = 64')
    ...
    parser.add_argument("--max-new-tokens", type=int, default=128,
                        help='max generated tokens for official MME baseline decoding')
    parser.add_argument("--official-mme-decoding", action=argparse.BooleanOptionalAction, default=True,
                        help='use common public MME baseline decoding settings when no pruning is enabled')
    parser.add_argument("--calculation-script", type=str, default="./src/HiMAP/eval_tool/calculation.py",
                        help='path to calculation.py')
    parser.add_argument("--calculation-results-dir", type=str, default=None,
                        help='directory to save txt files compatible with calculation.py')
    ...
    if args.use_hmap_v == True:
        ...
        model.config.jsd_entropy_use_dynamic_boundaries = args.jsd_entropy_use_dynamic_boundaries
    ...
    is_pruning_mode = args.use_hmap_v or args.use_fast_v or args.use_jsd_entropy_prune
    official_decoding_enabled = args.official_mme_decoding and (not is_pruning_mode)
    gen_use_cache = True if official_decoding_enabled else False
    gen_output_attentions = False if official_decoding_enabled else True
    gen_max_new_tokens = args.max_new_tokens if official_decoding_enabled else max(args.max_new_tokens, 1024)
    print(f"[Decode] conv_mode={args.conv_mode}, official_mme_decoding={official_decoding_enabled}, use_cache={gen_use_cache}, max_new_tokens={gen_max_new_tokens}")
    ...
    mode_name = 'himap' if args.use_hmap_v else ('jsd_entropy' if args.use_jsd_entropy_prune else ('fastv' if args.use_fast_v else 'baseline'))

    calc_output_dir = args.calculation_results_dir
    if calc_output_dir is None:
        calc_output_dir = f"mme_calc_inputs_{mode_name}"
    calc_output_dir = os.path.abspath(os.path.expanduser(calc_output_dir))

    export_for_calculation(results, calc_output_dir)
    print(f"\ncalculation.py 输入文件已导出到: {calc_output_dir}")

    calculation_script = os.path.abspath(os.path.expanduser(args.calculation_script))
    if not os.path.exists(calculation_script):
        raise FileNotFoundError(f"calculation.py not found: {calculation_script}")
    run_calculation(calculation_script, calc_output_dir)
    ...
    final_results = {
        ...
        'calculation_results_dir': calc_output_dir,
    ...
    # 保存结果
    output_file = f"mme_results_{mode_name}.json"
