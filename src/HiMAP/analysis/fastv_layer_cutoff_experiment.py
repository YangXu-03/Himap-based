...
def run_experiment(args):
    """
    实验目标：测试从第2层到最后一层，在每一层完全剪除图像token的准确率
    """
    # Model
    disable_torch_init()
...
def plot_results(results, output_plot):
    ...
    plt.title('FastV: Impact of Complete Image Token Removal at Different Layers', fontsize=14)
    ...
    # 优先使用显式 baseline（results['baseline_acc']），若不存在则回退到第一个点
    if len(accs) > 0:
        ...
        if baseline_for_drop > 0:
            ...
            plt.title(f'FastV: Relative Performance Drop (Baseline Acc={baseline_for_drop:.4f})', fontsize=14)
            plt.grid(True, alpha=0.3)
            plt.legend(fontsize=11)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="FastV Layer Cutoff Experiment on ScienceQA")
    ...
    # Output files
    parser.add_argument('--output-file', type=str, default='fastv_layer_cutoff_results.json',
                        help="Output JSON file for results")
    parser.add_argument('--output-plot', type=str, default='fastv_layer_cutoff_plot.png',
