...
# 优先使用本仓库的 LLaVA 实现，避免导入到其它项目的同名包
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
LLAVA_ROOT = os.path.normpath(os.path.join(CURRENT_DIR, "..", "..", "LLaVA"))
...
if __name__ == "__main__":
    ...
    print(f"PyTorch可见的GPU数量: {torch.cuda.device_count()}")
    if torch.cuda.is_available():
        print(f"当前使用的GPU: {torch.cuda.current_device()}")
        print(f"GPU名称: {torch.cuda.get_device_name(0)}")
        print(f"注意: 由于CUDA_VISIBLE_DEVICES=1，物理GPU 1被映射为逻辑GPU 0")
    ...
    model.model.reset_hmapv()  # 注释掉不存在的方法
    ...
    for i, line in enumerate(tqdm(questions)):
        ...
        # 立即清理中间变量
        del output_ids, pred_logit, loss
