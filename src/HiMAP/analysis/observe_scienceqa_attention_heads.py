"""
LLaVA 模型在 ScienceQA 数据集上的注意力头可视化分析脚本

功能描述：
    观测 LLaVA 多模态大模型在 ScienceQA 数据集上推理时，各个注意力头（Attention Heads）
...
可视化输出：
    ...
    - X轴: 32个注意力头
    - Y轴: 平均注意力值
    - 颜色: 蓝色(T2V)、橙色(V2T)、绿色(Total) 分别显示

使用方法：
    python observe_scienceqa_attention_heads.py \\
        --model-path liuhaotian/llava-v1.5-7b \\
        --image-folder /path/to/scienceqa/images \\
        --question-file /path/to/scienceqa.json \\
...
def main():
    parser = argparse.ArgumentParser(description="Observe attention heads in LLaVA on ScienceQA dataset")
    ...
    # 保存原始记录
    records_path = os.path.join(args.output_dir, "scienceqa_attention_head_records.pt")
    torch.save(records, records_path)
    print(f"Saved raw attention records to {records_path}")
    
    # 收集所有头的值
    all_layers = collect_all_head_values(records)
    
    # 绘制图表
    print("\nGenerating plots...")
    plot_stacked_hist_overall(all_layers, args.output_dir, "scienceqa", len(records))
    plot_stacked_hist_per_layer(all_layers, args.output_dir, "scienceqa")
