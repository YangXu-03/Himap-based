...
def attention_share(attn: torch.Tensor, query_idx: List[int], key_idx: List[int]) -> float:
    """Return average attention mass from queries to keys."""
...
def compute_token_groups(conv, prompt: str, tokenizer, input_ids: torch.Tensor, attentions: List[torch.Tensor]):
    """Get index lists for system, text, and visual tokens after image expansion."""
...
def process_sample(model, tokenizer, image_processor, line: Dict, args, device: torch.device):
    ...
    # Prepare prompt with image token placeholder
...
def average_by_category(records: List[Dict]):
    ...
    for cat, items in grouped.items():
        means, layers = average_attentions(items)
        cat_means[cat] = means
    return cat_means, layers
...
def plot_overall(means: Dict[str, List[float]], layers: List[int], output_dir: str):
    ...
    plt.title("MME Attention Breakdown (overall)")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    path = os.path.join(output_dir, "mme_attention_overall.png")
    plt.savefig(path, dpi=300, bbox_inches="tight")
    plt.close()
    return path


def plot_by_category(cat_means: Dict[str, Dict[str, List[float]]], layers: List[int], output_dir: str):
    if not cat_means:
        return None
    ...
    for idx, cat in enumerate(cats):
        ax = axes[idx]
        for k in PLOT_TYPES:
            ax.plot(layers, cat_means[cat].get(k, []), label=TYPE_LABELS[k], linewidth=1.5)
        ax.set_title(cat)
    ...
    fig.suptitle("MME Attention Breakdown by Category", y=0.98)
    plt.tight_layout()
    path = os.path.join(output_dir, "mme_attention_by_category.png")
    plt.savefig(path, dpi=300, bbox_inches="tight")
    plt.close()
    return path
...
def main():
    ...
    torch.save(records, os.path.join(args.output_dir, "mme_attention_records.pt"))

    means, layers = average_attentions(records)
    overall_plot = plot_overall(means, layers, args.output_dir)
    cat_means, layers = average_by_category(records)
    cat_plot = plot_by_category(cat_means, layers, args.output_dir)

    print(f"Saved {len(records)} samples of attention stats to {args.output_dir}")
    print(f"Overall plot: {overall_plot}")
    if cat_plot:
        print(f"Category plot: {cat_plot}")
