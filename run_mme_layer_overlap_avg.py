...
from layer_overlap_utils import (
    get_prompt_image_received_scores,
    get_topk_index_set,
    resolve_attention_spans,
)

def load_mme_samples(mme_json_path, samples_per_category=None):
    """
    Load MME data and group samples by category.
    Returns a dictionary: {category: [sample_item, ...]}

    If `samples_per_category` is provided and > 0, each category keeps at most
    that many samples.
    """
    ...
    for item in data:
        ...
        if samples_per_category is None or samples_per_category <= 0 or len(samples_by_category[cat]) < samples_per_category:
            samples_by_category[cat].append(item)

    selected_sample_count = sum(len(samples) for samples in samples_by_category.values())
    limit_desc = "all samples" if samples_per_category is None or samples_per_category <= 0 else f"up to {samples_per_category} samples/category"
    print(
        f"Selected {len(samples_by_category)} categories, {selected_sample_count} samples total ({limit_desc}) from {len(data)} samples."
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of samples to run for each category. Use 0 or a negative value to use all samples.",
    )
    ...
    output_dir = os.path.join(project_root, "mme_layer_overlap_avg_results")
    ...
    # Standard LLaVA-1.5 uses 576 visual tokens.
    image_token_len = 576
    fallback_sys_length = getattr(model.config, "fast_v_sys_length", 35)
    
    # Get grouped samples
    samples_by_category = load_mme_samples(
        mme_json_path,
    ...
    for category_idx, (category, samples) in enumerate(
        ...
        for sample in sample_iterator:
            ...
            for layer_idx, layer_attn in enumerate(attentions):
                ...
                # 按需求统计：每个 image token 接收到的 image+prompt 注意力总和。
                # 即对 query=[image tokens, prompt tokens]、key=[image tokens]
                # 的注意力矩阵在 head/query 维度求和，再对 image token 做 top-k。
                image_attn_scores = get_prompt_image_received_scores(
                    layer_attn,
                    image_start,
                    image_end,
                    prompt_start,
                ...
                idx_set1, top1_idx, top1_vals = get_topk_index_set(image_attn_scores, k_1pct, return_details=True)
                top1_indices[layer_idx] = idx_set1
                
                idx_set10, top10_idx, top10_vals = get_topk_index_set(image_attn_scores, k_10pct, return_details=True)
                top10_indices[layer_idx] = idx_set10

                # 计算并记录本层分数方差等诊断信息
                scores_cpu = image_attn_scores.detach().cpu()
                sample_diag["layers"][layer_idx] = {
                    "variance": float(scores_cpu.var()),
                    "mean": float(scores_cpu.mean()),
                    "min": float(scores_cpu.min()),
                    "max": float(scores_cpu.max()),
                    "top10_indices": top10_idx,
                    "top10_values": top10_vals
                }

            if not valid_attn:
                continue
                
            category_diagnostics.append(sample_diag)

            # Compute overlap matrices for sample
        ...
        if sample_count > 0:
            avg_matrix_1 = accum_matrix_1 / sample_count
            avg_matrix_10 = accum_matrix_10 / sample_count
            ...
            # 保存诊断信息
            diag_file = os.path.join(output_dir, f"{safe_cat}_diagnostics.json")
            with open(diag_file, 'w') as f:
                json.dump(category_diagnostics, f, indent=2)
