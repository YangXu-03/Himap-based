...
def main():
    ...
    parser.add_argument("--model-dtype", type=str, default="bfloat16", choices=["float32", "bfloat16", "float16"],
    ...
    parser.add_argument("--force-eager-attn", action="store_true",
                        help="Force eager attention backend when collecting output_attentions")
    ...
    if args.force_eager_attn and hasattr(model, "config"):
        # Avoid backend-specific issues when output_attentions=True.
        try:
            model.config._attn_implementation = "eager"
            print("Attention backend forced to eager")
        except Exception as e:
            print(f"Warning: failed to force eager attention backend: {e}")
    ...
    # Standard LLaVA-1.5 uses 576 visual tokens after image expansion.
    image_token_len = 576
    ...
    for category, sample in tqdm(sample_items, desc="Processing categories"):
        ...
        image_token_positions = (input_ids[0] == IMAGE_TOKEN_INDEX).nonzero(as_tuple=False).flatten()
        if image_token_positions.numel() == 0:
            print(f"Warning: IMAGE_TOKEN_INDEX not found in prompt for category {category}. Skipping.")
            continue
        image_token_start = int(image_token_positions[0].item())
        image_token_end = image_token_start + image_token_len
        print(
            f"[{category}] image token span in expanded sequence: "
            f"start={image_token_start}, end={image_token_end}, prompt_len={input_ids.shape[1]}"
        )
        ...
        for layer_idx, layer_attn in enumerate(attentions):
            ...
            # Extract image part by dynamic image-token anchor, not hardcoded system length.
            if token_importance.shape[0] < image_token_end:
                print(
                    f"Warning: Sequence length {token_importance.shape[0]} too short for category {category}. "
                    f"Expected at least {image_token_end}. Skipping."
                )
                break

            image_attn_scores = token_importance[image_token_start:image_token_end]
            layer_diag["image_attn_scores"] = tensor_nan_stats(image_attn_scores)

            if layer_idx == 0 and torch.all(image_attn_scores == 0):
                print(
                    f"[ZeroAttentionWarning] category={category} layer=0 image_attn_scores are all zeros. "
                    f"span=({image_token_start}, {image_token_end})"
                )
            
            # Get indices with highest attention scores using topk
            top1_vals_t, top1_indices_t = torch.topk(image_attn_scores, k_1pct)
            layer_diag["top30_values"] = tensor_nan_stats(top1_vals_t)
        ...
        if args.save_all_token_attn:
            save_full_token_attentions(
                attentions=attentions,
                input_ids=input_ids,
                tokenizer=tokenizer,
                output_dir=output_dir,
                safe_cat=safe_cat,
                save_all_heads=args.save_all_heads_attn,
                save_heatmaps=args.save_full_token_heatmaps,
            )
