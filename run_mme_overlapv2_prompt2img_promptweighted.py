...
if __name__ == "__main__":
    ...
    parser.add_argument("--heatmap-save-dir", type=str, default="./attn_heatmaps_promptweighted", help="注意力重叠率热力图保存路径")
            ...
            if hasattr(forward_outputs, "attentions") and forward_outputs.attentions is not None:
                ...
                if prompt_indices.numel() == 0:
                    ...
                    for l in range(num_layers):
                        attn = prefill_attns[l][0]  # [num_heads, seq_len, seq_len]
                        ...
                        # 1) 提取 prompt->prompt 注意力，计算每个 prompt token 接收注意力总和并归一化为重要性向量。
                        #    prompt_to_prompt_attn: [num_heads, num_prompt_tokens, num_prompt_tokens]
                        prompt_to_prompt_attn = attn[:, prompt_indices, prompt_indices]
                        if has_nan:
                            prompt_to_prompt_attn = torch.nan_to_num(prompt_to_prompt_attn, nan=0.0)

                        # 对 query 维和 head 维求和，得到每个 key(prompt token)收到的总注意力。
                        # prompt_importance_raw: [num_prompt_tokens]
                        prompt_importance_raw = prompt_to_prompt_attn.sum(dim=0).sum(dim=0)
                        ...
                        # 2) 用 prompt 重要性向量加权 prompt->image 注意力，得到每个 image token 的加权总注意力。
                        #    prompt_to_img_attn: [num_heads, num_prompt_tokens, num_image_tokens]
                        ...
                        # 先对 head 做平均，再按 prompt_importance 做加权和。
                        # mean_prompt_to_img_attn: [num_prompt_tokens, num_image_tokens]
                        mean_prompt_to_img_attn = prompt_to_img_attn.mean(dim=0)
                        # weighted_img_attn: [num_image_tokens]
                        weighted_img_attn = (mean_prompt_to_img_attn * prompt_importance.unsqueeze(-1)).sum(dim=0)

                        k = max(1, int(math.ceil(weighted_img_attn.shape[0] * args.topk_img_token_percent / 100.0)))
                        if k > 0:
                            _, topk_inds = torch.topk(weighted_img_attn, k)
                    ...
                    if valid_layers > 0:
                        ...
                        plt.title(
                            f"PromptWeighted Prompt->Image Token Top-{k_percent_str}% Overlap (k={selected_k})\\nCategory: {category}",
                            fontsize=14,
                        )
                        ...
                        save_path = os.path.join(
                            ...
                            f"heatmap_promptweighted_{safe_cat_name}_top{safe_k_percent}pct.png",
                        )
